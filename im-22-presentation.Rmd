...
p1 <- df_result %>% 
  mutate(across(!n_sim, function(x) x - `Treat none`)) %>%
  select(-`Treat none`) %>%
  fx_newplot() +
  xlab("INB (Treat None as ref.)")

p2 <- df_result %>% 
  mutate(across(!n_sim, function(x) x - `Treat all`)) %>%
  # select(-c(2:3)) %>%
  select(-`Treat all`) %>%
  fx_newplot() +
  xlab("INB (Treat All as ref.)")
...
data.frame(name=names(df_result)[-1]) %>%
  left_join(., df_percent_best, by="name") %>%
  mutate(
    n=ifelse(is.na(n), 0, n),
    `%`=scales::percent(n/nrow(df_result))
  ) %>%
  arrange(desc(n)) %>%
  rename("Method"=name) %>%
  kable() %>%
